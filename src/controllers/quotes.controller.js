...
/*
  Cliente: aceptar propuesta de cotización.

  Reglas:
  - solo el dueño de la cotización puede aceptarla
  - solo se puede aceptar si ya existe propuesta generada
  - al aceptar:
      status -> aprobada
      isAcceptedByClient -> true
      acceptedAt -> fecha actual
*/
...
export const createQuote = catchAsync(async (req, res) => {
  ...
  if (!projectType || !estimatedBudget || !estimatedTime || !location) {
    throw new AppError("Faltan datos de consulta.", 400);
  }

  let service = null;
  let resolvedServiceTitle = null;

  if (serviceId) {
    service = await dbGetServiceById(serviceId);
  ...
  if (!service && specificService) {
    resolvedServiceTitle = specificService;
  }

  if (!resolvedServiceTitle) {
    throw new AppError("Debes seleccionar un servicio válido.", 400);
  }
  ...
  const newQuote = await dbCreateQuote({
    ...
    specificService: !service ? resolvedServiceTitle : undefined,
    estimatedBudget,
    location,
    estimatedTime,
    description,
